...
import { WOODFRAME_REGIME_LINE, compileDeck } from '../src/woodframe/train/compile';
...
import type { BuildingSpec, BunkerSpec, StructureSpec } from '../src/woodframe/spec';
...
test('every card has a size, a length, a count and a reference — no half-filled backs', () => {
  for (const card of deckFor(familyById('gp-frame')!.preset).cards) {
    ...
    assert.equal(card.back.regimeLine, WOODFRAME_REGIME_LINE, `${card.id}: the (PH) line travels with the card`);
  }
});
