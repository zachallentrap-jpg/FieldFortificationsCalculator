...
test('CSV uses . decimals and no thousands separators regardless of locale', () => {
  ...
  // A genuinely large total (255 overhead-cover sandbags × 999) must appear bare, never grouped.
  ...
  assert.ok(big.includes('254745'), 'large totals are ungrouped');
  assert.ok(!big.includes('254,745'), 'no thousands separator inside a number');
});
