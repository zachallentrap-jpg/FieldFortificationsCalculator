...
/** The front leg face, in plan, at a given height — the frame's own batter curve. */
function legFaceZ(spec: { cabPlanFt: number; platformHeightFt: number }, y: number): number {
  const half = spec.cabPlanFt / 2;
  const batter = TOWER.batterPerSideFt.value as number;
  const cx = half + batter;
  const h = Math.max(1e-9, spec.platformHeightFt);
  return cx - (half + batter * (1 - Math.min(1, Math.max(0, y / h))));
}
...
test('THE LADDER CLEARS THE BATTERED FRAME AT EVERY RUNG, not just at the deck', () => {
  ...
  assert.ok(rungs.length > 4, 'the preset climbs on a ladder');
  const gaps = rungs.map((r) => legFaceZ(spec, r.position[1]!) - r.position[2]!);
  for (let i = 0; i < rungs.length; i++) {
    assert.ok(gaps[i]! >= want - 1e-9,
      `${rungs[i]!.id} at y=${rungs[i]!.position[1]!.toFixed(1)} stands ${(gaps[i]! * 12).toFixed(2)} in `
  ...
  assert.ok(spread < 1e-9, `the gap wanders by ${(spread * 12).toFixed(3)} in — the ladder is not parallel to the frame`);
});
...
test('the ladder still reaches the landing, and its rails still run past it', () => {
  ...
  // Rail length is the raked climb PLUS the raked extension.
  const batter = TOWER.batterPerSideFt.value as number;
  const lean = batter / deckY;
