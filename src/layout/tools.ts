...
export function scenariosOverlay(scenarios: Scenario[], activeId: string | null): string {
  ...
  return (
    '<div class="tools"><h2>Scenarios</h2>' +
...
export function missionOverlay(result: MissionResult, count: number): string {
  ...
  if (count === 0) {
    return '<div class="tools"><h2>Mission BOM</h2><p class="empty">Add positions to roll up one bill of materials, then enter on-hand quantities to see shortfalls.</p>' + addClear + '</div>';
  }
  ...
  return (
    '<div class="tools"><h2>Mission BOM — ' + result.totalPositions + ' position(s)</h2>' + addClear +
...
export function compareOverlay(results: Result[]): string {
  if (results.length === 0) {
    return '<div class="tools"><h2>Compare</h2><p class="empty">Add 2–3 configurations to compare them side by side.</p><div class="tool-actions"><button type="button" class="btn" data-action="compare-add">Add current</button></div></div>';
  }
...
export function planOverlay(plan: PlanResult | null, hours: number, team: number): string {
  ...
  if (!plan) {
    return '<div class="tools"><h2>Time-available planning</h2><p class="empty">Given hours and team size, what standard can you actually build for the current position + threat?</p>' + form + '</div>';
  }
  ...
  return (
    '<div class="tools"><h2>Time-available planning</h2>' + form +
