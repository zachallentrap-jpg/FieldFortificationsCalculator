...
OUT = os.path.join(HERE, "led_house_day.png")
...
def warm_window():
    # daytime exterior glass: dark, glossy, reflects the sky — reads as real glass
    m = bpy.data.materials.new("glass" + str(random.random()))
    m.use_nodes = True
    b = next(n for n in m.node_tree.nodes if n.type == 'BSDF_PRINCIPLED')
    b.inputs['Base Color'].default_value = (0.02, 0.03, 0.045, 1.0)
...
# daytime: fixture lamps are OFF — dark glass domes, not emitters
MAT_PORCH    = simple("porch", (0.05, 0.05, 0.06), rough=0.15)
MAT_PATH     = simple("path",  (0.06, 0.06, 0.07), rough=0.2)
...
def tree(x, y, h, spread):
    bpy.ops.mesh.primitive_cone_add(radius1=h*0.045, radius2=h*0.018, depth=h*0.6, location=(x, y, h*0.30))
...
tree(-13.5, -11, 7.5, 2.2)     # left, grounded in the lawn, trunk visible
tree(15.5, -1.0, 8.5, 2.6)     # right foreground beside the drive, fully in frame
tree(-19, 3, 10.0, 3.0)        # far-left backdrop
...
for bi, entries in buckets.items():
    ...
    # daytime: brighter emitter so the dot still reads against sunlit trim, no wall-wash twin
    sphere_cloud("led_%d" % bi, pts, 0.038, emissive("ledm_%d" % bi, (r, g, b), 12.0))
...
SUN_ELEV = 34.0   # degrees above horizon — late-afternoon warmth
SUN_AZ   = 62.0   # degrees — front-left of the facade, for raking shadows
...
# Directional key light matched to the Nishita sun position -> crisp warm shadows.
sun_data = bpy.data.lights.new("sun", 'SUN')
sun_data.energy = 3.2
sun_data.color = (1.0, 0.94, 0.82)
sun_data.angle = math.radians(1.5)  # soft-edged shadows
sun = bpy.data.objects.new("sun", sun_data)
bpy.context.collection.objects.link(sun)
# aim: elevation from +Z down, azimuth around Z
